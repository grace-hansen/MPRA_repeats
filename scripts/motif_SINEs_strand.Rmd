...
motif_locs$motif_center<-motif_locs$motif_start+(motif_locs$motif_stop-motif_locs$motif_start)/2
...
## How much overlap is there between sequences with shared motifs and sequences with Alus?
dat_motif_Alu<-nrow(dat[grepl("Alu",dat$rep_subtype) & dat$motif_start!=0,]) #How many sequences have both a shared motif and a Alu?
...
knitr::kable(observed,caption="Observed prevalence of motifs and Alus, all sequences")
knitr::kable(expected,caption="Expected prevalence of motifs and Alus, all sequences")
...
EMVars_motif_Alu$motif_Alu_dist[EMVars_motif_Alu$rep_strand=="+"]<-EMVars_motif_Alu$motif_center[EMVars_motif_Alu$rep_strand=="+"]-EMVars_motif_Alu$rep_start[EMVars_motif_Alu$rep_strand=="+"]
EMVars_motif_Alu$motif_Alu_dist[EMVars_motif_Alu$rep_strand=="-"]<-EMVars_motif_Alu$rep_stop[EMVars_motif_Alu$rep_strand=="-"]-EMVars_motif_Alu$motif_stop[EMVars_motif_Alu$rep_strand=="-"]
ggplot(EMVars_motif_Alu)+
  geom_histogram(aes(x=motif_Alu_dist),binwidth=1)+
  scale_x_continuous(name="motif center - Alu start")+
  theme_minimal()+
  ggtitle("EMVars")

enhancers_motif_Alu<-enhancers[grepl("Alu",enhancers$rep_subtype) & enhancers$motif_start!=0,]
enhancers_motif_Alu$motif_Alu_dist[enhancers_motif_Alu$rep_strand=="+"]<-enhancers_motif_Alu$motif_center[enhancers_motif_Alu$rep_strand=="+"]-enhancers_motif_Alu$rep_start[enhancers_motif_Alu$rep_strand=="+"]
enhancers_motif_Alu$motif_Alu_dist[enhancers_motif_Alu$rep_strand=="-"]<-enhancers_motif_Alu$rep_stop[enhancers_motif_Alu$rep_strand=="-"]-enhancers_motif_Alu$motif_stop[enhancers_motif_Alu$rep_strand=="-"]
...
nonsig_motif_Alu$motif_Alu_dist[nonsig_motif_Alu$rep_strand=="+"]<-nonsig_motif_Alu$motif_center[nonsig_motif_Alu$rep_strand=="+"]-nonsig_motif_Alu$rep_start[nonsig_motif_Alu$rep_strand=="+"]
nonsig_motif_Alu$motif_Alu_dist[nonsig_motif_Alu$rep_strand=="-"]<-nonsig_motif_Alu$rep_stop[nonsig_motif_Alu$rep_strand=="-"]-nonsig_motif_Alu$motif_stop[nonsig_motif_Alu$rep_strand=="-"]
...
sig_motif_Alu<-dat[grepl("Alu",dat$rep_subtype) & dat$motif_start!=0 & dat$type!="nonsig",]
sig_motif_Alu$motif_Alu_dist[sig_motif_Alu$rep_strand=="+"]<-sig_motif_Alu$motif_center[sig_motif_Alu$rep_strand=="+"]-sig_motif_Alu$rep_start[sig_motif_Alu$rep_strand=="+"]
sig_motif_Alu$motif_Alu_dist[sig_motif_Alu$rep_strand=="-"]<-sig_motif_Alu$rep_stop[sig_motif_Alu$rep_strand=="-"]-sig_motif_Alu$motif_stop[sig_motif_Alu$rep_strand=="-"]
...
D<-ggplot(plot_dat)+
  ...
  scale_x_continuous(name="Distance from Alu start to motif center")+
  ...
  ggtitle("")
D
pdf("~/midway/MPRA_repeats/Alus_to_motif_distance.pdf",width=10,height=4)
D
dev.off()
...
active_Alus<-sig_motif_Alu[sig_motif_Alu$motif_Alu_dist>50 & sig_motif_Alu$motif_Alu_dist<100,]
...
#Merge with repeats
Alus<-merge(repeats,Alu_bed,by=c("chr","rep_start","rep_stop"))

#Flip seq of reverse Alus to reverse complement
Alus$seq[Alus$rep_strand=="-"]<-chartr("ATGCN","TACGN",sapply(lapply(strsplit(Alus$seq[Alus$rep_strand=="-"], NULL), rev),paste, collapse=""))
Alus$motif<-grepl("GAGGTCA",substr(Alus$seq,65,80))
Alus$motif[Alus$motif==TRUE]<-"motif"
Alus$motif[Alus$motif==FALSE]<-"no motif"

barcodes<-as.data.frame(fread("~/projects/MPRA/WHR/results/barcode_activity.txt"))
barcodes<-merge(barcodes,Alus[,c("rsid","rep_start","rep_stop","pos","rep_type","rep_score","rep_strand","rep_subtype","type","seq","motif")],by="rsid")
barcodes$motif[barcodes$motif=="motif"]<-"Alus with GAGGTCA"
barcodes$motif[barcodes$motif=="no motif"]<-"Alus without GAGGTCA"
...
G<-ggplot()+
  ...
  annotate("text",x=0.45,y=2.5,label=paste("p=",formatC(GAGprop_test$p.value,digits = 3),sep=''),cex=6)+
  scale_fill_manual(values=c(darken(rgb(pony_colors[3,1:3])),darken(rgb(pony_colors[5,1:3])),darken(rgb(pony_colors[9,1:3]))))+
...
pdf("~/midway/MPRA_repeats/GAGGTCA_proportions.pdf",width=3,height=5)
G
dev.off()

#Plot the interaction of GAGGTCA and strand on expression
rsid_dat<-merge(rsid_dat,Alus[,c('rsid','rep_strand')],by="rsid")
rsid_dat<-unique(rsid_dat)
rsid_dat$motif<-paste(rsid_dat$motif,rsid_dat$rep_strand,sep=", ")
rsid_dat$motif[grepl("Alus without GAGGTCA",rsid_dat$motif)]<-"Alus without GAGGTCA"

p_motif<-t.test(rsid_dat$qnorm_avg[rsid_dat$motif=="Alus with GAGGTCA, +"],rsid_dat$qnorm_avg[rsid_dat$motif=="Alus without GAGGTCA"])$p.value
p_strand<-t.test(rsid_dat$qnorm_avg[rsid_dat$motif=="Alus with GAGGTCA, +"],rsid_dat$qnorm_avg[rsid_dat$motif=="Alus with GAGGTCA, -"])$p.value

print(paste("Alus with GAGGTCA plus vs. minus strand: p=",p_strand))
print(paste("Alus with GAGGTCA vs. Alus without GAGGTCA: p=",p_motif))
...
S<-ggplot(rsid_dat)+
  ...
  scale_fill_manual(values=c(darken(rgb(pony_colors[10,1:3])),lighten(rgb(pony_colors[10,1:3])),"gray90"))+
  theme_minimal()+
  theme(axis.text.x=element_text(angle=-45),
        ...
        legend.position = "none")
S
pdf("~/midway/MPRA_repeats/GAGGTCA_expression_strand.pdf",width=4,height=6)
S
dev.off()
